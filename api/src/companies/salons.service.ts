...
export class SalonsService {
	...
	async findAll(page: number, limit: number, search?: string) {
		try {
			...
			this.logger.error(
				`Error finding all salons: ${error.message}`,
				error.stack,
			);
	...
	async findById(id: string) {
		try {
			...
			this.logger.error(
				`Error finding salon by ID: ${error.message}`,
				error.stack,
			);
		...
		try {
			...
			if (existingSalon) {
				throw new ConflictException(
					`A salon with the name "${createSalonDto.name}" already exists for this owner`,
				);
			}
			...
			if (error.code === "P2002") {
				throw new ConflictException(
					"A salon with this name or website slug already exists",
				);
			}

			// Handle other errors
			throw new InternalServerErrorException(
				`Failed to create salon: ${error.message}`,
			);
		}
	}
	...
	async update(id: string, updateSalonDto: UpdateSalonDto) {
		try {
			...
			if (
				updateSalonDto.websiteSlug &&
				updateSalonDto.websiteSlug !== salon.websiteSlug
			) {
				const existingSlug = await this.prisma.salon.findFirst({
					where: {
				...
				if (existingSlug) {
					throw new ConflictException(
						"A salon with this website slug already exists",
					);
				}
			}
	...
	async findAllServices(
		...
		try {
			...
			this.logger.error(
				`Error finding services for salon ${salonId}: ${error.message}`,
				error.stack,
			);
	...
	async findServiceById(
		...
		try {
			const service = await this.serviceRepository.findByIdForSalon(
				id,
				salonId,
			);
			...
			if (!service) {
				throw new NotFoundException(
					`Service with ID ${id} not found in salon ${salonId}`,
				);
			}
	...
	async createService(
		...
		try {
			...
			if (existingService) {
				throw new ConflictException(
					`A service with the name "${createServiceDto.name}" already exists in this salon`,
				);
			}
	...
	async updateService(
		...
		try {
			...
			if (updateServiceDto.name && updateServiceDto.name !== service.name) {
				...
				if (existingService) {
					throw new ConflictException(
						`A service with the name "${updateServiceDto.name}" already exists in this salon`,
					);
				}
			}
	...
	async removeService(id: string): Promise<{ id: string; isActive?: boolean }> {
		try {
			const service = await this.serviceRepository.findById(id);
			...
			this.logger.error(
				`Error removing service: ${error.message}`,
				error.stack,
			);
	...
	async getWorkingHours(salonId: string) {
		try {
			...
			const workingHours =
				await this.workingHoursRepository.findBySalonId(salonId);
			...
			this.logger.error(
				`Error getting working hours: ${error.message}`,
				error.stack,
			);
	...
	async setWorkingHours(salonId: string, workingHoursDto: WorkingHoursDto[]) {
		try {
			...
			this.logger.error(
				`Error setting working hours: ${error.message}`,
				error.stack,
			);
	...
	private validateWorkingHours(workingHours: WorkingHoursDto[]) {
		...
		for (const hours of workingHours) {
			...
			if (!hours.openTime || !hours.closeTime) {
				throw new BadRequestException(
					`For open days, openTime and closeTime are required for day ${hours.dayOfWeek}`,
				);
			}
			...
			if (openTime >= closeTime) {
				throw new BadRequestException(
					`Open time must be before close time for day ${hours.dayOfWeek}`,
				);
			}
		}
	}
