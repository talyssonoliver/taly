...
export class PaypalProvider implements IPaymentProvider {
  ...
  async processPaymentWithSavedMethod(
    ...
    try {
      this.logger.log(Processing PayPal payment with saved method for payment );
  ...
  async refundPayment(
    ...
    try {
      this.logger.log(Processing PayPal refund for transaction );
      ...
      this.logger.error(PayPal refund processing error: );
      throw error;
    }
  }
  ...
  async validateToken(token: string): Promise<any> {
    try {
      ...
      this.logger.error(PayPal token validation error: );
      throw error;
    }
  }
  ...
  private async getCaptureIdFromOrder(orderId: string): Promise<string | null> {
    try {
      ...
      this.logger.error(Error getting capture ID: );
      return null;
    }
  }
}
