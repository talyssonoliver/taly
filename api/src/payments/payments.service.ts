...
export class PaymentsService {
	...
	async findAll(
		...
		try {
			...
			this.logger.error(
				`Error finding all payments: ${error.message}`,
				error.stack,
			);
	...
	async findById(id: string) {
		try {
			...
			this.logger.error(
				`Error finding payment by ID: ${error.message}`,
				error.stack,
			);
	...
	async findByUserId(userId: string, page: number, limit: number) {
		try {
			...
			this.logger.error(
				`Error finding payments for user: ${error.message}`,
				error.stack,
			);
	...
	async create(createPaymentDto: CreatePaymentDto, userId: string) {
		try {
			const { appointmentId, amount, paymentMethod, provider } =
				createPaymentDto;
			...
			if (salon.ownerId !== userId && appointment.client.userId !== userId) {
				throw new UnauthorizedException(
					"You are not authorized to create a payment for this appointment",
				);
			}

			// Check if payment already exists for this appointment
			const existingPayment =
				await this.paymentRepository.findByAppointmentId(appointmentId);
	...
	async process(processPaymentDto: ProcessPaymentDto, userId: string) {
		try {
			const { paymentId, paymentMethodId, token, cardDetails } =
				processPaymentDto;
			...
			if (!appointment) {
				throw new NotFoundException(
					`Appointment with ID ${payment.appointmentId} not found`,
				);
			}

			const salon = appointment.salon;
			const isClientOrOwner =
				appointment.client.userId === userId || salon.ownerId === userId;
			...
			if (!paymentMethodId && !token && !cardDetails) {
				throw new BadRequestException(
					"Payment method, token, or card details required",
				);
			}
			...
			try {
				...
				this.logger.error(
					`Failed to send payment notification: ${notificationError.message}`,
			...
			try {
				if (processPaymentDto.paymentId) {
					await this.transactionRepository.create({
						...
						userId,
						amount:
							(await this.findById(processPaymentDto.paymentId))?.amount || 0,
						type: "PAYMENT",
						status: "FAILED",
				...
				this.logger.error(
					`Error recording failed transaction: ${recordError.message}`,
					recordError.stack,
				);
			}
	...
	async refund(refundPaymentDto: RefundPaymentDto, userId: string) {
		try {
			...
			if (appointment.salon.ownerId !== userId) {
				throw new UnauthorizedException(
					"Only salon owners can process refunds",
				);
			}
			...
			return this.prisma.$transaction(async (prisma) => {
				...
				await this.transactionRepository.create({
					...
					providerTransactionId: refundResult.transactionId, // Include provider transaction ID
				});
				...
				try {
					...
					this.logger.error(
						`Failed to send refund notification: ${notificationError.message}`,
						notificationError.stack,
					);
					// Don't fail the transaction if notification fails
				}
	...
	async deletePaymentMethod(id: string, userId: string) {
		try {
			...
			if (paymentMethod.isDefault) {
				...
				if (methods.length > 0) {
					await this.paymentMethodRepository.update(methods[0].id, {
						isDefault: true,
					});
				}
			}
	...
	async handlePaypalWebhook(event: any) {
		try {
			...
			this.logger.error(
				`Error handling PayPal webhook: ${error.message}`,
				error.stack,
			);
			throw error;
		}
		...
		try {
			...
			if (appointment) {
				try {
					...
					this.logger.error(
						`Failed to send payment webhook notification: ${notificationError.message}`,
					);
				}
			}
			...
			this.logger.error(
				`Error handling Stripe payment success: ${error.message}`,
				error.stack,
			);
	...
	private async handleStripePaymentFailure(paymentIntent: any) {
		try {
			...
			this.logger.error(
				`Error handling Stripe payment failure: ${error.message}`,
				error.stack,
			);
			return { error: error.message };
		}
	...
	private async handleStripeRefund(charge: any) {
		try {
			...
			if (!payment) {
				this.logger.warn(
					`Payment with transaction ID ${charge.payment_intent} not found`,
				);
				return { received: true };
			}
			...
			this.logger.error(
				`Error handling Stripe refund: ${error.message}`,
				error.stack,
			);
			return { error: error.message };
		}
		...
		try {
			...
			if (appointment) {
				try {
					...
					this.logger.error(
						`Failed to send PayPal payment webhook notification: ${notificationError.message}`,
					);
				}
			}
			...
			this.logger.error(
				`Error handling PayPal payment success: ${error.message}`,
				error.stack,
			);
			return { error: error.message };
		}
	...
	private async handlePaypalPaymentFailure(resource: any) {
		try {
			...
			this.logger.error(
				`Error handling PayPal payment failure: ${error.message}`,
				error.stack,
			);
			return { error: error.message };
		}
	...
	private async handlePaypalRefund(resource: any) {
		try {
			...
			const originalPaymentId = paymentLinks
				? paymentLinks.href.split("/").pop()
				: null;

			if (!originalPaymentId) {
				this.logger.warn(
					"Payment not found for PayPal refund - missing original payment ID",
				);
				return { received: true };
			}

			const payment =
				await this.paymentRepository.findByTransactionId(originalPaymentId);

			if (!payment) {
				this.logger.warn(
					`Payment not found for PayPal refund with transaction ID ${originalPaymentId}`,
				);
				return { received: true };
			}
			...
			this.logger.error(
				`Error handling PayPal refund: ${error.message}`,
				error.stack,
			);
			return { error: error.message };
		}
